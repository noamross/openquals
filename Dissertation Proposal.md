...
The model is a modification of the one used in @Cobb2012 and @Filipe2013.  Notably, 

$$\begin{aligned}
\boldsymbol{S}_{t+1} &\sim \text{Pois}\left[\boldsymbol{b(S_t+I_t)}\left(1-\sum_{i=1}^n w_i (S_it + I_it) \right) + \boldsymbol{rm_I I_t}\right] - \text{Binom}_1\left((1 - e^{(-\boldsymbol{\beta I_t} - \boldsymbol{\lambda_{ex}}})\boldsymbol{S_t} \right) -\text{Binom}_2(\boldsymbol{m_S} \boldsymbol{S_t}) \\
\boldsymbol{I}_{t+1} &\sim \text{Binom}_1\left((1 - e^{(-\boldsymbol{\beta I_t} - \boldsymbol{\lambda_{ex}}})\boldsymbol{S_t} \right) - \text{Binom}_3(\boldsymbol{m_I} \boldsymbol{S_t}) 
\end{aligned}$$

Modifications (a) the inclusion of demographic stochasticity, (b) conversion from continuous discrete time, (c) the inclusion $\lambda_ex$, force of infection from areas outside the system, and (d) the exclusion of recovery of infected trees, which has not been observed.
