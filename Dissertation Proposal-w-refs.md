...
The effect on such spatial structure on development of epidemics has been studied with continuous populations in space [@Bolker1999], metapopulation models (Park et al. (2001); Park et al. (2002)), models of individuals on a lattice (Filipe and Maule 2003, 2004) and discrete individuals in continuous space (Brown and Bolker (2004)). These approaches have reached similar conclusions of the effect of spatial structure in non-mobile populations. In all cases, the threshold of disease growth for a global outbreak ![(R\_0)](http://latex.codecogs.com/gif.latex?%28R_0%29 "(R_0)") is greater than the threshold for a local epidemic ![(R\_L)](http://latex.codecogs.com/gif.latex?%28R_L%29 "(R_L)"). For instance, in metapopulations on a lattice, ![R\_0 = R\_L (1 + z\\varepsilon)](http://latex.codecogs.com/gif.latex?R_0%20%3D%20R_L%20%281%20%2B%20z%5Cvarepsilon%29 "R_0 = R_L (1 + z\varepsilon)"), where ![z](http://latex.codecogs.com/gif.latex?z "z") is the average number of connections between patches and ![\\varepsilon](http://latex.codecogs.com/gif.latex?%5Cvarepsilon "\varepsilon") is the inter-patch contact rates (Park et al. 2001). In models of discrete individuals in space, ![R\_0 = \\lambda(1 + \\bar{\\mathcal{C}} SI)](http://latex.codecogs.com/gif.latex?R_0%20%3D%20%5Clambda%281%20%2B%20%5Cbar%7B%5Cmathcal%7BC%7D%7D%20SI%29 "R_0 = \lambda(1 + \bar{\mathcal{C}} SI)"), where ![\\lambda](http://latex.codecogs.com/gif.latex?%5Clambda "\lambda") would be the pathogen growth rate in a well-mixed population, and ![\\bar{\\mathcal{C}}](http://latex.codecogs.com/gif.latex?%5Cbar%7B%5Cmathcal%7BC%7D%7D "\bar{\mathcal{C}}") is the dispersal kernel-weighted spatial correlation between susceptible and infected individuals. ![\\bar{\\mathcal{C}}](http://latex.codecogs.com/gif.latex?%5Cbar%7B%5Cmathcal%7BC%7D%7D "\bar{\mathcal{C}}") evolves over time but reaches a minimum that represents the threshold required for a global epidemic (Brown and Bolker 2004). Also, both clustering and anti-clustering (oversdispersal) reduce the rate of increase of disease, except when clustering occurs at the scale of dispersal, in which case it can accellerate spread. Fat-tailed dispersal kernels accellerate spread, as well.
...
Here ![\\boldsymbol{S}\_t](http://latex.codecogs.com/gif.latex?%5Cboldsymbol%7BS%7D_t "\boldsymbol{S}_t") and ![\\boldsymbol{I}\_t](http://latex.codecogs.com/gif.latex?%5Cboldsymbol%7BI%7D_t "\boldsymbol{I}_t") are vectors of the populations of susceptible and infected individuals of each species, and ![\\boldsymbol{\\beta}](http://latex.codecogs.com/gif.latex?%5Cboldsymbol%7B%5Cbeta%7D "\boldsymbol{\beta}") is the matrix of contact rates. Other parameters are listed below in Table 1.

|Parameter Vector Symbol|Description|
|:----------------------|:----------|
|![\\boldsymbol m\_{S,I}](http://latex.codecogs.com/gif.latex?%5Cboldsymbol%20m_%7BS%2CI%7D "\boldsymbol m_{S,I}")|Probability of death per year, for both susceptible and infectious trees|
|![\\boldsymbol b](http://latex.codecogs.com/gif.latex?%5Cboldsymbol%20b "\boldsymbol b")|Fecundity per individual per year.|
|![\\boldsymbol r](http://latex.codecogs.com/gif.latex?%5Cboldsymbol%20r "\boldsymbol r")|Probability of resprouting after death by disease.|
|![\\boldsymbol w](http://latex.codecogs.com/gif.latex?%5Cboldsymbol%20w "\boldsymbol w")|Competitive coefficient (relative contribution to density-dependent recruitment)|
|![\\boldsymbol \\lambda\_{ex}](http://latex.codecogs.com/gif.latex?%5Cboldsymbol%20%5Clambda_%7Bex%7D "\boldsymbol \lambda_{ex}")|Rate of contact of each species with pathogen spores from outside the site|
...
The model is modified from Cobb et al. (2012) in several ways: (1) inclusion of demographic stochasticity, (2) conversion from continuous discrete time, (3) the inclusion of ![\\lambda\_ex](http://latex.codecogs.com/gif.latex?%5Clambda_ex "\lambda_ex"), force of infection from areas outside the system, (4) the exclusion of recovery of infected trees, which has not been observed in the field. Finally, the only population structure in Model A is the difference in species parameters. It excludes age structure, and spatial structure, assuming a well-mixed population and frequency-dependent transmission (Following Diekmann et al. (1990))
...
**Model B** adds stage structure to Model A. In this case, the vectors ![\\boldsymbol{S\_t}](http://latex.codecogs.com/gif.latex?%5Cboldsymbol%7BS_t%7D "\boldsymbol{S_t}") and ![\\boldsymbol{I\_t}](http://latex.codecogs.com/gif.latex?%5Cboldsymbol%7BI_t%7D "\boldsymbol{I_t}") represent the population divided by species and size class:
...
Where ![\\boldsymbol{A}](http://latex.codecogs.com/gif.latex?%5Cboldsymbol%7BA%7D "\boldsymbol{A}") is the matrix of demographic rates specifying transitions between classes and mortality. ![\\boldsymbol{A}](http://latex.codecogs.com/gif.latex?%5Cboldsymbol%7BA%7D "\boldsymbol{A}") is a block-diagonal matrix of size transition matrices, with no transitions between species classes.

**Models 3 and 4** modify Models 1 and 2 by adding spatial structure in the form of a lattice metapopulation. ![S\_t](http://latex.codecogs.com/gif.latex?S_t "S_t") and ![I\_t](http://latex.codecogs.com/gif.latex?I_t "I_t") become Individuals do not migrate between sub-populations on the lattice,
...
-   How does inclusion of each component of structure modify
    -   ![R\_0](http://latex.codecogs.com/gif.latex?R_0 "R_0") (global and local)
    -   Epidemic size
...
![ ](http://latex.codecogs.com/gif.latex?%20 " ")
 (Arulampalam et al. 2002)

Since ![p(X\_t | X\_{t-1})](http://latex.codecogs.com/gif.latex?p%28X_t%20%7C%20X_%7Bt-1%7D%29 "p(X_t | X_{t-1})") is unknown, It's value is determined at each time step by simulation.

PFMCMC requires two components to the model. A **process model** which simulates the states ![(X\_t)](http://latex.codecogs.com/gif.latex?%28X_t%29 "(X_t)") of the model, and an **observation model** of the observations at each time point, given underlying states ![(Y\_t | X\_t)](http://latex.codecogs.com/gif.latex?%28Y_t%20%7C%20X_t%29 "(Y_t | X_t)"). Unlike the process model, the observation model must have a tractable likelihood.
...
![\\begin{aligned} p\_OI &= p(\\text{Observing that tree is diseased}|\\text{tree is diseased}) p\_OS &= p(\\text{Observing that tree is healty}|\\text{tree is healthy}) \\end{aligned}](http://latex.codecogs.com/gif.latex?%5Cbegin%7Baligned%7D%0A%20p_OI%20%26%3D%20p%28%5Ctext%7BObserving%20that%20tree%20is%20diseased%7D%7C%5Ctext%7Btree%20is%20diseased%7D%29%0A%20p_OS%20%26%3D%20p%28%5Ctext%7BObserving%20that%20tree%20is%20healty%7D%7C%5Ctext%7Btree%20is%20healthy%7D%29%0A%5Cend%7Baligned%7D "\begin{aligned}
...
With an estimate of likelihood available, one needs an approach to determine the maximum-likelihood estimate of the model parameters ![(\\theta)](http://latex.codecogs.com/gif.latex?%28%5Ctheta%29 "(\theta)"). Several methods are available. **Iterated filtering** (IF) estimates ![\\theta](http://latex.codecogs.com/gif.latex?%5Ctheta "\theta") by replacing constant parameter values with a random walk ![\\theta\_t](http://latex.codecogs.com/gif.latex?%5Ctheta_t "\theta_t") of decreasing variance over time, and determining the "states" of this walk as the variance approaches zero. IF is computationally efficient but requires long time series.
...
![\\text{DIC} = D(\\bar\\theta) + 2p\_D](http://latex.codecogs.com/gif.latex?%5Ctext%7BDIC%7D%20%3D%20D%28%5Cbar%5Ctheta%29%20%2B%202p_D "\text{DIC} = D(\bar\theta) + 2p_D")

where ![p\_D](http://latex.codecogs.com/gif.latex?p_D "p_D") is a metric of model complexity (effective number of parameters) and ![D](http://latex.codecogs.com/gif.latex?D "D") is the deviance (-2 ![\\times](http://latex.codecogs.com/gif.latex?%5Ctimes "\times") log likelihood). ![p\_D](http://latex.codecogs.com/gif.latex?p_D "p_D") defined as

![p\_D = \\overline{D(\\theta)} - D(\\bar{\\theta})](http://latex.codecogs.com/gif.latex?p_D%20%3D%20%5Coverline%7BD%28%5Ctheta%29%7D%20-%20D%28%5Cbar%7B%5Ctheta%7D%29 "p_D = \overline{D(\theta)} - D(\bar{\theta})")

![p\_D](http://latex.codecogs.com/gif.latex?p_D "p_D") may be redfined as ![\\frac{p\_D}{n}](http://latex.codecogs.com/gif.latex?%5Cfrac%7Bp_D%7D%7Bn%7D "\frac{p_D}{n}") to correct for the amount of data.

DIC provides a measure of fit and complexity. It has two primary advantages over traditional measures of fit such as BIC and AIC. First, since ![p\_D](http://latex.codecogs.com/gif.latex?p_D "p_D") is based on the likelihood distribution, it incorporates information from the prior. Effectively, it reduces the effective number of parameters when they are restricted due to the prior. Secondly, the same property allows the effective number of parameters from random variables to be included.
...
-   The best-fit model provides us with estimates of the probability over disease outbreaks in a forest stand over a management period. This probability is contingent on both forest composition and spatial structure, and the burden of spores from other areas, and changes over time as forest dynamics change the host population.
    ...
    This becomes n optimal control problem in which ![c\_1](http://latex.codecogs.com/gif.latex?c_1 "c_1") is the annual cost of reducing the probability of disease arrival ![\\Lambda\_t](http://latex.codecogs.com/gif.latex?%5CLambda_t "\Lambda_t"), and ![c\_2](http://latex.codecogs.com/gif.latex?c_2 "c_2") is the cost of silvicultural treatment in any year of cutting.
...
Cutting events are discrete, periodic treatments that change forest composition. A cut can reduce the density of any class and species to either the levels associated with minimum probability of outbreak, or to minimum socially acceptable densities. Tanoak and bay laurel trees have little timber value (@REF), so each cut has a one-time cost, ![c\_1](http://latex.codecogs.com/gif.latex?c_1 "c_1"). For the purpose of this model ![c\_1](http://latex.codecogs.com/gif.latex?c_1 "c_1") is independent of the extent of density reduction.

![c\_2](http://latex.codecogs.com/gif.latex?c_2 "c_2") represents the combined annual costs of measures to reduce the external force of infection ![\\lambda\_{\\text{ex}}](http://latex.codecogs.com/gif.latex?%5Clambda_%7B%5Ctext%7Bex%7D%7D "\lambda_{\text{ex}}").

This model could be optimized by defining a function for the value of ecosystem services flowing annually from the forest annually as a function of the forest composition ![f(N\_t)](http://latex.codecogs.com/gif.latex?f%28N_t%29 "f(N_t)") [@Hartmann1976]. However, due to the difficulty of defining and valuing these services, it is more feasible to define constraints that represent the range of socially acceptable outcomes.

The constraint that makes most sense is a *minimum acceeptable density of the species of interest* ![(N\_{\\min})](http://latex.codecogs.com/gif.latex?%28N_%7B%5Cmin%7D%29 "(N_{\min})"). This value could be further specified to include, for instance, a minimum density of trees of a minimum size. Since the system is stochastic, this constraint expressed by a *level of acceptable risk*, the probability that the species density will fall beneath this value ![(p\_{\\text{acc}} \> p(N\_t \< N\_{\\min})](http://latex.codecogs.com/gif.latex?%28p_%7B%5Ctext%7Bacc%7D%7D%20%3E%20p%28N_t%20%3C%20N_%7B%5Cmin%7D%29 "(p_{\text{acc}} > p(N_t < N_{\min})").

The dynamic optimization problem is to minimize the cost of treatment while maintaining the outbreak probability below ![p\_{\\text{acc}}](http://latex.codecogs.com/gif.latex?p_%7B%5Ctext%7Bacc%7D%7D "p_{\text{acc}}"):

![\\min \\sum\_1\^T c\_1 + c\_2(\\lambda\_{ex}) \\text{ s.t. } p(N\_t \< N\_{\\min}) \< p\_{text{acc}}](http://latex.codecogs.com/gif.latex?%5Cmin%20%5Csum_1%5ET%20c_1%20%2B%20c_2%28%5Clambda_%7Bex%7D%29%20%5Ctext%7B%20s.t.%20%7D%20p%28N_t%20%3C%20N_%7B%5Cmin%7D%29%20%3C%20p_%7Btext%7Bacc%7D%7D "\min \sum_1^T c_1 + c_2(\lambda_{ex}) \text{ s.t. } p(N_t < N_{\min}) < p_{text{acc}}")
...
![\\min sum\_1\^T p\_{\\text{outbreak} \\text{ s.t. } \\sum\_0\^T c\_1 + c\_2(\\lambda\_{ex}) \\leq B\_T](http://latex.codecogs.com/gif.latex?%5Cmin%20sum_1%5ET%20p_%7B%5Ctext%7Boutbreak%7D%20%5Ctext%7B%20s.t.%20%7D%20%5Csum_0%5ET%20c_1%20%2B%20c_2%28%5Clambda_%7Bex%7D%29%20%5Cleq%20B_T "\min sum_1^T p_{\text{outbreak} \text{ s.t. } \sum_0^T c_1 + c_2(\lambda_{ex}) \leq B_T")

or

![\\min sum\_1\^T p\_{\\text{outbreak} \\text{ s.t. } c\_1 + c\_2(\\lambda\_{ex}) \\leq B\_{\\text{ann}} \\forall T](http://latex.codecogs.com/gif.latex?%5Cmin%20sum_1%5ET%20p_%7B%5Ctext%7Boutbreak%7D%20%5Ctext%7B%20s.t.%20%7D%20c_1%20%2B%20c_2%28%5Clambda_%7Bex%7D%29%20%5Cleq%20B_%7B%5Ctext%7Bann%7D%7D%20%5Cforall%20T "\min sum_1^T p_{\text{outbreak} \text{ s.t. } c_1 + c_2(\lambda_{ex}) \leq B_{\text{ann}} \forall T")
